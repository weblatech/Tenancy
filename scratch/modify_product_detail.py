...
file_path = r"d:\Projects\saas-ecommerce\resources\views\tenant\product-detail.blade.php"
...
try:
    ...
    # 4. Update product review buttons
    content = content.replace("bg-[#16a34a] hover:bg-green-700 text-white font-black py-4 px-10 rounded-xl transition shadow-lg hover:-translate-y-0.5 w-full md:w-auto text-center text-sm uppercase tracking-wider", "btn-secondary-custom font-black py-4 px-10 rounded-xl transition shadow-lg hover:-translate-y-0.5 w-full md:w-auto text-center text-sm uppercase tracking-wider")
    content = content.replace("bg-gray-950 text-white font-bold py-3.5 px-8 rounded-xl hover:bg-black transition text-sm shadow-md", "btn-primary-custom font-bold py-3.5 px-8 rounded-xl transition text-sm shadow-md")

    with open(file_path, 'w', encoding='utf-8') as f:
        f.write(content)
        
    print("Successfully modified product-detail.blade.php")

except Exception as e:
    print(f"Error modifying product-detail: {e}")
