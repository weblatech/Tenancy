...
<body class="bg-slate-50 min-h-screen flex flex-col justify-center py-12 sm:px-6 lg:px-8 font-sans">
    ...
    <div class="sm:mx-auto sm:w-full sm:max-w-md text-center">
        ...
        <h2 class="mt-6 text-2xl font-black text-slate-800 leading-tight">
            {{ $settings->enable_rtl ? 'اپنے اکاؤنٹ میں لاگ ان کریں' : 'Log in to your account' }}
        </h2>
        <p class="mt-2 text-xs font-semibold text-slate-450">
            {{ $settings->enable_rtl ? 'یا' : 'Or' }}
            <a href="/customer/register" class="text-indigo-600 hover:text-indigo-500 font-extrabold hover:underline">
                {{ $settings->enable_rtl ? 'نیا اکاؤنٹ بنائیں' : 'create a new account' }}
            </a>
        </p>
    </div>
    ...
    <div class="mt-8 sm:mx-auto sm:w-full sm:max-w-md">
        <div class="bg-white py-8 px-4 shadow-xl border border-slate-100 sm:rounded-3xl sm:px-10">
            ...
            @if(session('success'))
                <div class="mb-5 p-4 bg-emerald-50 border border-emerald-250 text-emerald-700 text-xs font-bold rounded-2xl flex items-center gap-2">
                    <span>✅</span> {{ session('success') }}
                </div>
            @endif

            <form action="/customer/login" method="POST" class="space-y-6">
                @csrf

                <div>
                    <label for="email" class="block text-[10px] font-black uppercase text-slate-500 tracking-wider mb-2">
                        {{ $settings->enable_rtl ? 'ای میل ایڈریس' : 'Email Address' }}
                    </label>
                ...
                <div>
                    <label for="password" class="block text-[10px] font-black uppercase text-slate-500 tracking-wider mb-2">
                        {{ $settings->enable_rtl ? 'پاس ورڈ' : 'Password' }}
                    </label>
                ...
                <div class="flex items-center justify-between">
                    <div class="flex items-center">
                        ...
                        <label for="remember" class="ml-2 block text-xs font-bold text-slate-500">
                            {{ $settings->enable_rtl ? 'مجھے یاد رکھیں' : 'Remember me' }}
                        </label>
                    </div>
                </div>
                ...
                <div>
                    <button type="submit" class="btn-primary-custom w-full font-black py-4 px-6 rounded-xl transition shadow-lg flex items-center justify-center gap-2 text-xs uppercase tracking-wider">
                        <span>{{ $settings->enable_rtl ? 'لاگ ان کریں' : 'Log In' }}</span>
